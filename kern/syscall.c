...
sys_exofork(void)
{
	...
	int retval;
	if((retval = env_alloc(&new_env, curenv->env_id)) < 0)
		return retval;
...
sys_env_set_status(envid_t envid, int status)
{
	...
	if(status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE)
		return -E_INVAL;
	struct Env* new_env = NULL;
	int retval = envid2env(envid, &new_env, 1);
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	struct Env *env = NULL;
	int retval = envid2env(envid, &env, 1);
	if(retval < 0) return retval;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((uint32_t)va >= UTOP || ROUNDDOWN(va, PGSIZE) != va) return -E_INVAL;
	...
	retval = page_insert(new_env->env_pgdir, new_page, va, perm);
	if(retval < 0) {
		page_free(new_page);
		return retval;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if((uint32_t)srcenvid >= UTOP || (uint32_t)dstenvid >= UTOP) return -E_INVAL;
	if(ROUNDDOWN(srcva, PGSIZE) != srcva || ROUNDDOWN(dstva, PGSIZE) != dstva)
		return -E_INVAL;
	if(!((perm & (PTE_U | PTE_P)) && !(perm & ~PTE_SYSCALL)))
		return -E_INVAL;
	struct Env *src_env = NULL;
	struct Env *dst_env = NULL;
	int retval = envid2env(srcenvid, &src_env, 1);
	if(retval < 0) return retval;
	retval = envid2env(dstenvid, &dst_env, 1);
	if(retval < 0) return retval;
	pte_t *pte = NULL;
	struct PageInfo *page = page_lookup(src_env->env_pgdir, srcva, &pte);
	if(page == NULL) return -E_INVAL;
	if((perm & PTE_W) && !(*pte & PTE_W)) return -E_INVAL;
	retval = page_insert(dst_env->env_pgdir, page, dstva, perm);
	if(retval < 0) return retval;
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	int retval = envid2env(envid, &dstenv, 0);
	if(retval < 0) return retval;
